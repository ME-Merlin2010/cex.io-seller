...
public class Reinvestor extends CexAPI {
    ...
    public Reinvestor(String user, String key, String secret) {
        ...
        this.apiCalls = 0;
        this.coin = new Coin(true, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, "XLM/EUR");
        this.done = false;
    ...
    public void loadSettings() {
        ...
        if (file.exists() && file.isFile()) {
            try {
                ...
                this.coin.active = Boolean.valueOf(settings[3]);
                this.coin.reserve = BigDecimal.valueOf(Double.valueOf(settings[4]));
                this.coin.max = BigDecimal.valueOf(Double.valueOf(settings[5]));
                this.coin.min = BigDecimal.valueOf(Double.valueOf(settings[6]));
    ...
    public void saveSettings() {
        ...
        try {
            write = new PrintWriter(new BufferedWriter(new FileWriter(
                "settings.txt", false)));
            String settings = this.username + "," + this.apiKey + ","
                + this.apiSecret + "," + this.coin.active + ","
                + this.coin.reserve.toPlainString() + ","
                + this.coin.max.toPlainString() + ","
                + this.coin.min.toPlainString();
    ...
    class InputThread extends Thread {
        ...
        public void prompt() {
            if (this.user.gui == null) {
                out("\n[B]alance | [1] Display 'XLM/EUR' | [E]xit\n"
        ...
        public void run() {
            ...
            while (!this.user.done && !this.stop) {
                ...
                try {
                    ...
                    // active, pending
                    trade_coin = ((this.user.coin.active)
                        && (this.user.balance != null) && (this.user.coin.reserve
                        .compareTo(this.user.balance.XLM.available) <= -1))
                        || ((this.user.coin.active) && (!this.user.pending
                            .isEmpty()));
                    ...
                    if (trade_coin) {
                        ...
                        if (trade_coin) {
                            this.analyze(this.user.balance.XLM, this.user.coin);
                        }
                    } else {
                        if (this.user.debug) {
                            this.user
        ...
        public void analyze(Currency currency, Coin coin) {
                    ...
                    if (temp > (this.user.pending.get(a).time + 60000)) {
                        ...
                        if (canceled) {
                            ...
                            out("Reinvestor: Canceled pending order (ID: "
                                ...
                                + tempOrder.pending.toPlainString() + " EUR "
            ...
            if (currency.available.compareTo(coin.reserve) == 1) {
                Ticker price = new Gson().fromJson(this.user.execute("ticker",
                    new String[] { coin.pairTicker }), Ticker.class);

                // if price range is within user specified limits: purchase
                if (((coin.max.compareTo(BigDecimal.ZERO) == 0) || (coin.max.compareTo(price.last) == 1))
                    && ((coin.min.compareTo(BigDecimal.ZERO) == 0) || (price.last.compareTo(coin.min)) == 1)) {
                    // calculate amount to buy
                    BigDecimal amount = (currency.available.subtract(coin.reserve)).divide(price.last, 8, RoundingMode.DOWN);

                    // subtract 0.25% trading fee (accounting for the new fee increase)
                    BigDecimal fee = (amount.multiply(new BigDecimal("0.0025"))
                        .multiply(price.last)).setScale(8, RoundingMode.UP);
                    amount = ((currency.available.subtract(coin.reserve))
                        .subtract(fee))
                        .divide(price.last, 8, RoundingMode.DOWN);

                    if (amount.compareTo(new BigDecimal(0.00000001)) == 1) {
                    	// TODO Hier weitermachen!
                    	System.err.println("Im here!");
                    	// Place-Order: Pair-Ticker: XLM/EUR buy Amount: 277.04654900 Price: 0.313
                    	System.err.println("Place-Order: Pair-Ticker: " + coin.pairTicker + " buy Amount: " +  amount.toPlainString() + " Price: " + price.last.toPlainString() );
                        Order order = new Gson().fromJson(
                        		this.user.execute("place_order", new String[] { 
                        			coin.pairTicker, "buy", amount.toPlainString(), price.last.toPlainString() }), Order.class);
                        ...
                        if (order.error == "") {
                            ...
                            // tell purchase error @ coin value for x EUR
                    ...
                    out("Reinvestor: The current price of a " + coin.pairTicker
